...
**The backfill (`znhstry backfill`) re-fetches historical report pages for per-player rows.** Paced at `BACKFILL_INTERVAL` (10 s) to stay polite, newest first, with a 5-hour budget per run. Four cron windows per day at 01:15, 07:15, 13:15 and 19:15 UTC, dispatched by the same Cloudflare Worker; the 19:15 window ends by 00:15, clear of the 00:45 nightly. Pages that fail to parse are recorded in `skipped.txt` with a reason (`unavailable` or `unparsed`) so they are not retried. `backfill_state.json` carries remaining count and is read by the workflow's gate. The `battlestats-players` concurrency group serializes the backfill and the nightly's player writes; `cancel-in-progress` is false so a 5-hour run is not killed by a nightly firing. Remove the four backfill cron lines from the Worker once remaining hits 0.
