...
**The backfill (`znhstry backfill`) re-fetches historical report pages for per-player rows.** Paced at `BACKFILL_INTERVAL` (10 s) to stay polite, newest first, with a 5-hour budget per run. Not scheduled: `backfill.yml` runs only on `workflow_dispatch`. Adding four cron windows (01:15, 07:15, 13:15, 19:15 UTC) to the Cloudflare Worker made its deploy fail, so the lines were removed until the cause is settled; a window must end before the 00:45 nightly. Pages that fail to parse are recorded in `skipped.txt` with a reason (`unavailable` or `unparsed`) so they are not retried. `backfill_state.json` carries remaining count and is read by the workflow's gate. The `battlestats-players` concurrency group serializes the backfill and the nightly's player writes; `cancel-in-progress` is false so a 5-hour run is not killed by a nightly firing. Remove the four backfill cron lines from the Worker once remaining hits 0.
