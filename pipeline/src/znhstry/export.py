...
def _export_zones(con: duckdb.DuckDBPyConnection, out: Path) -> dict[str, Any]:
    ...
    up per zone at runtime. They cost 1.35 MB on 1.6M zones because they gzip
    into long runs: region ids are grouped by country upstream and the file is
    in zone_id order.
    """
...
def _export_events(con: duckdb.DuckDBPyConnection, out: Path) -> list[dict[str, Any]]:
    ...
    Rows group by zone rather than running in global timestamp order. Grouping
    a zone's trajectory together is what makes the stream compress - 4x versus
    3.1x for timestamp order - and the client rebuilds a day index on load in
    milliseconds.

    The tiebreak within a zone is `observed_at`, not `activity_date`. 653,071
    zone-days carry more than one event, so ordering by the date alone leaves
    them tied and DuckDB's parallel sort emits them in whatever order it
    finishes in. That made the export nondeterministic between identical runs
    -- 945 shards differed -- and, worse, wrong: the client takes the last row
    in file order as the zone's state for that day, so an arbitrary order can
    surface an earlier observation as the day's outcome. (zone_id, observed_at)
    is unique across all 9,869,428 rows, so this is a total order.
    """
    months = con.execute("""
