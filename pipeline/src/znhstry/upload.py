...
which is why the payloads live here rather than in the site bundle:
    ...
    Cache-Control           Every shard is immutable once written - its name
                            changes when its contents do - so a repeat visit
                            re-downloads nothing. meta.json is the one file
                            that must be allowed to go stale, since it is how
                            a client discovers the rest.
...
def _put(s3, bucket: str, path: Path, key: str) -> int:
    ...
    s3.put_object(
        ...
        CacheControl=MANIFEST if path.name.endswith("meta.json") else IMMUTABLE,
        **extra,
    )
    return len(body)
